...
Vue tarayıcı geliştirici araçları (devtools) eklentisi; bir Vue uygulamasının bileşen ağacını keşfetmenizi, tek tek bileşenlerin durumunu incelemenizi, durum yönetimi olaylarını izlemenizi ve performansı profilleme yapmanızı sağlar.
...
Ana yazı: [Vue'yu TypeScript ile Kullanma](/guide/typescript/overview).
...
Ana yazı: [Test Rehberi](/guide/scaling-up/testing).
...
- [Vitest](https://vitest.dev/), Vue / Vite ekibi üyeleri tarafından oluşturulmuş, hıza odaklanan bir test koşucusudur (test runner). Özellikle Vite tabanlı uygulamalarda birim / bileşen testleri için aynı anlık geri bildirim döngüsünü sağlayacak şekilde tasarlanmıştır.

- [Jest](https://jestjs.io/), [vite-jest](https://github.com/sodatea/vite-jest) üzerinden Vite ile çalışacak şekilde uyarlanabilir. Ancak bu yalnızca Vite tabanlı bir kuruluma taşımanız gereken mevcut Jest tabanlı test paketleriniz varsa önerilir; aksi halde Vitest, çok daha verimli bir entegrasyonla benzer işlevler sağlar.
...
Özel bloklar (custom blocks), aynı Vue dosyasına farklı istek sorgularıyla yapılan içe aktarımlar halinde derlenir. Bu içe aktarım isteklerini ele almak, kullanılan derleme aracına kalmıştır.
...
Bu paket, Vue çekirdek monorepo'sunun bir parçasıdır ve her zaman ana `vue` paketiyle aynı sürümde yayımlanır. Ana `vue` paketine bağımlılık olarak dahil edilir ve `vue/compiler-sfc` üzerinden proxy'lenir; bu nedenle ayrıca yüklemenize gerek yoktur.

Paketin kendisi, Vue SFC'leri işlemek için alt seviye yardımcı araçlar sağlar ve yalnızca özel araçlarında Vue SFC desteği sunmak isteyen araç geliştiricileri için tasarlanmıştır.
